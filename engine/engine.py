"""
ICC Cloud Engine — Build 2 (behavioural memory)
Scans 6 symbols, matches setups against RAW historical outcomes including
their PATH (peak, giveback, bars-to-peak), and manages focus trades with
history+momentum thresholds blended over fixed safety floors.
...
def manage_focus(state, analyses):
    ...
    f["last_r"] = r
    # ---- history+momentum thresholds (blended over fixed floors) ----
    b = f.get("behav", {})
    n_raw = b.get("n_raw", 0)
    ...
    # ---- path tracking for the managed record ----
    ...
    if not msgs:
        ...
        if f["pings"] >= 8:
            f["pings"] = 0
            msgs.append(f"POSITION OPEN — SYNCING | R {r:+.2f} | MFE {f['mfe']:+.2f} | peak {f.get('peak_r', 0):.2f}")
    if msgs:
...
def handle_ack(con, state, data):
    if data in ("T|take", "T|risky"):
        ...
        tg_send(f"🔒 FOCUS MODE — tracking {name} {d} ({f['tier']})\n"
                ...
                f"Management = history + momentum. Routine alerts muted except A+ break-glass.",
                [[{"text": "🏁 CLOSED — log it", "callback_data": "C|close"}]])
    elif data == "T|no":
        state["pending"] = None
    ...
    if txt.startswith("/taken"):
        ...
        if len(parts) >= 6:
            ...
            state["focus"] = {"symbol": sym, "tf": "manual", "direction": d, "tier": tier,
                              ...
                              "mfe": 0.0, "mae": 0.0, "peak_r": 0.0, "giveback": 0.0,
                              "bars_held": 0, "behav": {},
                              "be_done": False, "reduced": False, "extended": False, "pings": 0}
